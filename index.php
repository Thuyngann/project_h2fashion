        ...
        case 'confirmbill':
            ...
            case 'huydh':
                if (isset($_GET['id']) && !empty($_GET['id'])) {
                    ...
                    if ($bill_trangthai == 0) {
                        update_dh($idbill, $trangthai);
                        // var_dump($bill_trangthai); die;
                        // header("Location: index.php?act=mybill&loi=".$loi);


                    } else {
                        // Không cho phép hủy đơn hàng
                        $loi =  "Không thể hủy đơn hàng " ;
                        header("Location: index.php?act=ls_bill&id=$idbill&loi=".$loi);
